...
class Eval_thread():
    ...
    def run(self):
        ...
        [mae, max_f, s] = map(lambda x: round(x, 6), [mae, max_f, s])

        f = open(self.output_dir + 'result.txt', "a+")
        f.write(
            '\n {} dataset with {} method get mae={:.6f}, max-fmeasure={:.6f}, max-Emeasure={:.6f}, S-measure={:.6f}.\n'
                .format(self.dataset, self.method, mae, max_f, max_e, s))
        
        return '[cost:{:.6f}s]{} dataset with {} method get mae={:.6f}, max-fmeasure={:.6f}, max-Emeasure={:.6f}, ' \
               'S-measure={:.6f}.'.format(time.time()-start_time, self.dataset, self.method, mae, max_f, max_e, s)
    
    def Eval_mae(self):
        # print('eval[MAE]:{} dataset with {} method.'.format(self.dataset, self.method))
        avg_mae, img_num = 0.0, 0.0
        with torch.no_grad():
            for pred, gt, mask in self.loader:
                if self.cuda:
                    pred = pred.to(device)
                    gt = gt.to(device)
                    mask = mask.to(device)
